...
CREATE NETWORK RULE IF NOT EXISTS SNOWFLAKE_LEARNING_DB.PUBLIC.CUSTOM_API_NETWORK_RULE
  MODE = EGRESS
  TYPE = HOST_PORT
  VALUE_LIST = (
    'rest.isric.org',          -- For SoilGrids soil properties
    'archive-api.open-meteo.com', -- For Precipitation, Wind Speed, Wind Direction
    'api.opentopodata.org',       -- For Topography/Elevation
    'overpass-api.de'             -- For Land Use and River Networks (OpenStreetMap)
  );

CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION CUSTOM_EXTERNAL_ACCESS
  ALLOWED_NETWORK_RULES = (
    SNOWFLAKE_LEARNING_DB.PUBLIC.CUSTOM_API_NETWORK_RULE
  )
  ENABLED = TRUE;

-- Verify integration creation
DESCRIBE INTEGRATION CUSTOM_EXTERNAL_ACCESS;
